...
// export function AddToCart({ variants, availableForSale }: { variants: marketplaceProductType[]; availableForSale: boolean }) {
//   const router = useRouter();
//   const searchParams = useSearchParams();
//   const [isPending, startTransition] = useTransition();
//   // const defaultVariantId = variants.length === 1 ? variants[0]?.id : undefined;
//   const defaultVariantId = variants[0].id;
//   // const variant = variants.find((variant: marketplaceProductType) =>
//   //   variant.selectedOptions.every((option) => option.value === searchParams.get(option.name.toLowerCase()))
//   // );
//   // const selectedVariantId = variant?.id || defaultVariantId;
//   // const title = !availableForSale ? 'Out of stock' : !selectedVariantId ? 'Please select options' : undefined;
