...
export default async function CairoNewsPage() {
  console.time('getLatestCategoryArticles');
  const news = await getLatestCategoryArticles('cairo', 30).then((res) => res[0].articles);
  console.timeEnd('getLatestCategoryArticles');
  console.log('inside CairoNewsPage');
  if (!news) return null; 

  return (
    <div className="flex flex-col w-full max-w-6xl m-auto gap-4 py-8 px-4">
      <h1 className="text-3xl font-bold animate-fadeIn">أخبار القاهرة - مصر</h1>
