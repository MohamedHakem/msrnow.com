...
export default async function FaiyumNewsPage() {
  ...
  const news = await getLatestCategoryArticles('faiyum', 30).then((res) => res[0].articles);
  console.timeEnd('getLatestCategoryArticles');
  console.log('inside FaiyumNewsPage');
  if (!news) return null;

  return (
    <div className="flex flex-col gap-4 py-8 px-4">
      <h1 className="text-3xl font-bold animate-fadeIn">أخبار الفيوم - مصر</h1>
