...
export default async function ArtsNewsPage() {
  console.time('getLatestCategoryArticles');
  const news = await getLatestCategoryArticles('arts', 30).then((res) => res[0].articles);
  console.timeEnd('getLatestCategoryArticles');
  console.log('inside ArtsNewsPage');
  if (!news) return null;

  return (
    <div className="flex flex-col gap-4 py-8 px-4">
      <h1 className="text-3xl font-bold animate-fadeIn">أخبار الفنون</h1>
