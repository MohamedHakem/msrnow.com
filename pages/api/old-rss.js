...
export default async function handler(req, res) {
  ...
  // const categories = ["egypt", "sports", "politics", "business", "world"]
  // I only have "egypt" / homepage ready for now
  const categories = [""]

  const rssItems = await scrapeRss(categories)
  ...
  rssItems.forEach((item) => {
    ...
    <link>https://www.msrnow.com${item.link}</link>
    <guid isPermaLink="true">https://www.msrnow.com${item.link}</guid>
    <pubDate>${formatDate(item.datetime)}</pubDate>
    <content:encoded><![CDATA[ <img 
      src="${item.img}" /> ]]>
      </content:encoded>
    <image>
    <url>${item.img.replace("&", "&amp;")}</url>
    </image>
    <media:content url="${item.img.replace(
      "&",
      "&amp;"
